...
class Concurrency extends EventEmitter {
   ...
   */
	runTestEnvironments(envs, args, availColors) {
		return Promise.all(envs.map((environment, index) => {
			const childProcess = this.createChildProcess(environment, args, ["--env", environment], index);
   ...
   */
	runMultipleTestWorkers(modules, args, availColors) {
		const maxWorkerCount = this.getTestWorkersCount();
		...
		return new Promise((resolve, reject) => {
			Concurrency.buildProcessQueue(maxWorkerCount, modules, (modulePath, index, next) => {
				const outputLabel = Utils.getModuleKey(modulePath, this.settings.src_folders, modules);
				const childProcess = this.createChildProcess(outputLabel, args, ["--test", modulePath, "--test-worker"], index);
   ...
   */
	static buildProcessQueue(concurrency, modules, fn) {
		const maxWorkers = Math.min(concurrency, modules.length);
