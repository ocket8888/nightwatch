...
class Transport extends EventEmitter {
   ...
   */
	resolveElement(result, element, multipleElements) {
		...
		if (multipleElements && Array.isArray(value) && value.length > 0) {
			if (value.length > 1) {
				const message = `More than one element (${value.length}) found for element <${element.toString()}> with selector: "${element.selector}".`;
   ...
   */
	...
	getElementNotFoundResult(result) {
		...
		const errorId = this.Errors.StatusCode && this.Errors.StatusCode.NO_SUCH_ELEMENT;
		const errorInfo = this.Errors.findErrorById(errorId);
