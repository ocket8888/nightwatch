const {WebElement, By, RelativeBy} = require("selenium-webdriver");
...
class Element {
   ...
   */
	...
	toString() {
		...
		const prefix = this.constructor === Element ? "@" : "";
   ...
   */
	getRecursiveLookupElement() {
		const lookupList = getAncestorsWithElement(this);
   ...
   */
	static applyFiltering(element, resultElements) {
		if (Element.requiresFiltering(element)) {
			const foundElem = resultElements[element.index];
...
function getDescription(instance) {
	if (instance.name) {
		const classType = instance.constructor.name.toLowerCase(); // Element, Section or any other subclass's name
